...
function product_list()
{
    ...
    global $list;
    $results_per_page = 8;
    $query = "Select * from `products` order by product_price $list";
    ...
    $query = "Select * from `products` order by product_price $list LIMIT " . $page_first_result . ',' . $results_per_page;
    $result_query = mysqli_query($connect, $query);
    ...
    for ($page = 1; $page <= $number_of_page; $page++) {
        ...
        if (!isset($_SESSION['email'])) {
            ...
            <div style='display: flex;' class='" . $class . "'>
                <a class='pagination-item__link' href='filter_price.php?list=$list&page=" . $page . "'>" . $page . "</a>
            </div>
        ";
        } else {
            echo "
